...
def main():
    parser = argparse.ArgumentParser(
        description="Continual setup with nested adapter (freeze backbone, new head)."
    )
    ...
    parser.add_argument(
        "--unfreeze-top",
        action="store_true",
        help="Also fine-tune the last conv block during Task2.",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoints/continual.pth",
        help="Where to save the trained model.",
    )
    parser.add_argument(
        "--results-path",
        type=str,
        default="continual_results.txt",
    ...
    for epoch in range(args.epochs_task1):
        ...
        print(f"[Continual][Task1] Epoch {epoch+1}/{args.epochs_task1} - loss {loss:.4f} acc {acc:.4f}")

    _, acc_t1_before = evaluate(backbone, head_t1, test_t1, device)

    # Freeze backbone; optionally unfreeze top block.
    set_requires_grad(backbone, False)
    if args.unfreeze_top:
        set_requires_grad(backbone.block3, True)

    adapter_t2 = Adapter().to(device)
    head_t2 = ClassifierHead().to(device)

    params = list(adapter_t2.parameters()) + list(head_t2.parameters())
    if args.unfreeze_top:
        params += list(backbone.block3.parameters())

    optimizer_t2 = optim.Adam(params, lr=args.lr)
    for epoch in range(args.epochs_task2):
        loss, acc = train_one_epoch(
            backbone, head_t2, train_t2, optimizer_t2, criterion, device, adapter_t2
        )
        print(f"[Continual][Task2] Epoch {epoch+1}/{args.epochs_task2} - loss {loss:.4f} acc {acc:.4f}")

    _, acc_t2 = evaluate(backbone, head_t2, test_t2, device, adapter_t2)
    ...
    torch.save(
        {
            ...
            "adapter_t2": adapter_t2.state_dict(),
            "history": history,
    ...
    print("\nFinal metrics (continual nested adapter):")
